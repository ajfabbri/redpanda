...
/// returned), or the abort source fires (in which case sleep_aborted exception
/// is returned), whichever comes first. In case of either timeout or abort the
/// result of the original future is ignored.
///
        ...
        state(typename Clock::time_point deadline, seastar::abort_source& as)
          : timer([this] {
            ...
            auto maybe_sub = as.subscribe([this]() noexcept {
                timer.cancel();
                done.set_exception(seastar::sleep_aborted{});
            });
            if (!maybe_sub) {
                done.set_exception(seastar::sleep_aborted{});
            } else {
